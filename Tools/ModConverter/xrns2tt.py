#!/usr/bin/env python
#
# Renoise XRNS -> tracker-tone converter.
#
# Based on DuinoTune by Blake Livingston:
#   https://github.com/blakelivingston/DuinoTune
#
# Modified by Miguel Angel Exposito (2017-2020) to target the Melbits Pod
# firmware synth: extended voice/channel types (PCM, PWM, CMD, NOISE),
# expanded envelope handling, and integration with the Pod tooling.
#
# Upstream DuinoTune is published under its own terms — see the upstream
# repository for the canonical license. This adapted file inherits those
# terms; the Melbot-specific modifications are released under the same.
...
class Inst(object):
    env_tpl = """
  static const int16_t slopes_%d[] = {%s};
  static const uint8_t pticks_%d[] = {%s};
  static const synth_envelope_t env_%d = {
    .num_points = %d,
    .starting_level = %d,
  ...
  """
    inst_tpl = """
static const synth_instrument_t inst_%d = {
    .voice_type=%d,
    .duty=%d,
    .bitcrush=%d,
    .envelope=%s
    ...
    def parse_voice_from_name(self, vname):
        if len(vname.split('_')) >= 3:
            ...
            if duty in self.type_to_channel_type_enum:
                self.duty = self.type_to_channel_type_enum[duty]
            else:
    ...
    def parse(self, env, tpl, lpb):
        ...
        for i in range(1, len(levels)):
            ...
            if t_d > 0:
                self.slopes.append(int((int(d * 255) << 7) / t_d))
...
def compress_data(data_in):
    global outtxt

    with open ("temp.bin", "wb") as compdata:        
        _bytes = bytearray(data_in)
        compdata.write(_bytes)

    process = subprocess.Popen(['lzss.exe', 'e', 'temp.bin', 'out.bin'],
                        stdout=subprocess.PIPE, 
                        stderr=subprocess.PIPE)
    stdout, stderr = process.communicate()

    outtxt = outtxt + str(stdout) + "\n"
    ...
    with open("out.bin", 'rb') as f:
        while True:
            ...
            if len(block) == 0:
                ...
                for b in block:
                    #data_out.append(struct.unpack('<B', b))
                    data_out.append(int(b))
...
def xrns_to_tt(input_file,
    ...
    active_patterns.sort() # Ordered list of unique pattern indices found in 'SequenceEntries' XML node
    pattern_remap = dict((j, i) for i, j in enumerate(active_patterns)) # key = pattern index, value = index in active_patterns list

    sequence = [pattern_remap[s] for s in sequence] # Array of remapped pattern indices
    ...
    #PWM_BLINK               = 3
    #PWM_FADETO              = 4
    #PWM_SET_SOURCE_LEVEL    = 0
    #PWM_SET_TARGET_LEVEL    = 1
    #PWM_FADEOUT             = 6 
    ...
    def mkcode(voice, code):
        return ((voice & 0xf) << 4) + code # vvvvcccc
    ...
    for p_idx in range(len(patterns)):
        ...
        active_instruments = {}
        track_volumes = {}
        for l_idx in range(len(cur_pat)):       # Iterate lines in pattern
            tt_line = []    # Decoded lines
            cur_line = cur_pat[l_idx]
            
            for t_idx in range(len(cur_line)):  # Iterate tracks in line
                fxval = 0
                ...
                specialCmdChannel = None
                hasSpecialCmd = False

                if not note is None:                    
                    # Search for the M0 tag in the panning column
                    for elem in note:
                        if elem.tag == "Panning":
                            if elem.text == "M0":
                                hasSpecialCmd = True
                ...
                if hasSpecialCmd:
                    for elem in note:
                        if elem.tag == "Volume":
                            specialCmdChannel = int(elem.text, 16)
                    if specialCmdChannel is None:
                        continue
                    if fx is not None:
                        fxcode = None
                        for elem in fx:
                            if elem.tag == "Number":
                                fxcode = int(elem.text, 16)  # Opcode

                            if elem.tag == "Value":
                                fxval = int(elem.text, 16)   # Param
                        ...
                        #### CUSTOM COMMANDS ####
                        if fxcode2 == PWM_SET_LEVEL_ABS:
                            inst = inst_remap[specialCmdChannel]
                            
                            fxval >>= 1
                            if fxcode & 0x80: 
                                fxval |= 0x80

                            tcode = mkcode(inst, fxcode2)
                            tt_line.extend([tcode, fxval])
                        
                        if fxcode2 == PWM_FADEIN:
                            inst = inst_remap[specialCmdChannel]
                        ...
                        if fxcode2 == PWM_FADETO_ABS:
                            inst = inst_remap[specialCmdChannel]

                            fxval >>= 1
                            if fxcode & 0x80: fxval |= 0x80

                            tcode = mkcode(inst, fxcode2)
                            tt_line.extend([tcode, fxval])

                        if fxcode2 == CMD_PCM_PLAYSND:
                            inst = inst_remap[specialCmdChannel]
                    ...
                    if fx is not None:
                        ...
                        if fxcode == "0G": # GLIDE effect
                            ...
                            tcode = mkcode(t_idx, PORTAMENTO)   # Keep the glide going
                            tt_line.append(tcode)
                    if note is None:
                        continue
                    note_val = None
  ...
  static const uint8_t p_dat[] = {%s};
  static const uint16_t p_len[] = {%s};
  static const uint8_t p_ord[] = {%s};
  static const synth_instrument_t* instruments[] = {%s};  
  const synth_songDefinition_t %s = {
  .pattern_data = p_dat,
  .num_patterns = %d,
  .pattern_lengths = p_len,
  .pattern_order = p_ord,
  //.pattern_offsets = p_off,
  .bpm = %s,
  .rows_per_beat = %s,
  .ticks_per_row = %s,
  ...
  #include "Audio/synth.h"
  extern const synth_songDefinition_t %s;
#endif
  """
    ...
    for tt_pat in ttcode_patterns:
        ...
        uncompressed_data = uncompressed_data + tt_pat
        while tt_pat:
            tt_l = tt_pat[0:20]
            tt_pat = tt_pat[20:]
            tt_lines.append(', '.join(["0x%x" % d for d in tt_l]))            
        #uncompressed_data_str = uncompressed_data_str + (',\n '.join(tt_lines))
        pattern_output.append(tt_lines)

    #pattern_offsets = [len(ttcode_patterns[l]) for l in range(len(pattern_output))]

    compressed_data = compress_data(uncompressed_data)
    
    while compressed_data:
            tt_l = compressed_data[0:20]
            compressed_data = compressed_data[20:]
            compressed_data_str = compressed_data_str + (', '.join(["0x%x" % d for d in tt_l])) + ",\n"


    pat_defs = ""
    #pat_defs = "\n".join(["static const uint8_t pattern_%d[] = {\n%s\n};" % (pid, ',\n'.join(data))
    ...
    c_output = c_tmpl % (inst_defs, pat_defs, 
        #",\n".join(["pattern_%d" % i for i in range(len(pattern_output))]),        
        compressed_data_str,
