...
typedef struct
{
    ...
    HAL_analogSample_t lastMin, lastMax;
    HAL_analogSample_t lastSample;
    mldsp_State state;
...
static void mldsp_maxMinWindow(HAL_analogSample_t* buffer, uint16_t len, uint16_t idx, HAL_analogSample_t *pMin, HAL_analogSample_t* pMax)
{
    ...
    HAL_analogSample_t min = HALANALOGSAMPLE_MAX, max = HALANALOGSAMPLE_MIN;
    ...
    {
        HAL_analogSample_t cur = buffer[i];
        if(cur < min) min = cur;
        if(cur > max) max = cur;
    }
    *pMin = min;
...
static void mldsp_movingAvg(HAL_analogSample_t* buffer, uint16_t len, uint16_t idx, HAL_analogSample_t *avg)
{
    int32_t _avg = 0;
    int32_t startIdx, endIdx;    
